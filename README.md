# Coffer — a Grand Exchange flipping assistant
...
Premium adds crowd fill-rate + backtested confidence on picks and a **web dashboard** with an
interactive price chart and your positions. Off-client **Discord alerts** are available on every
tier, including Free.

## Getting started

1. Install **Coffer** from the Plugin Hub and enable it.
2. Open the Coffer side panel → **Create account** (free, takes a few seconds — username,
   password, and email).
3. Set your bankroll and GE slots, and start flipping. Coffer does the rest.

## Community & support

Questions, bugs, or feature ideas? Join the [Coffer Discord](https://discord.gg/zeBkPymf4f) — or use
**Send feedback** in the plugin's Settings.

## Tiers

- **Free** — F2P flip suggestions.
- **Standard** — full members tool: plan, positions + coaching, watchlist, alch/crash, P&L + log.
- **Premium** — crowd fill-rate + backtested confidence, and the web dashboard.

Off-client Discord alerts are available on every tier (Free, Standard, Premium).
...
fixed; there's nothing to configure beyond the in-panel login and a few flip preferences. Self-serve
password reset is available from the web dashboard ("Forgot password?" in the panel links there) for
accounts with an email on file; accounts without one should contact the owner.
